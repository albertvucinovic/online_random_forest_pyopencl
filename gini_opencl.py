...
#define hash(k) as_uint((k)) %% %(prime)d %% %(num_samples)d
...
#define FEATURES 1
#define LOCAL_MATRIX_SIZE SEMPLES*FEATURES
...
__kernel 
__attribute__((reqd_work_group_size(SAMPLES,FEATURES,1)))
void gini(
  __global float *A, 
  ...
  __global float *gini_res)
{
  ...
  main_index=thread_sample*%(num_features)d+thread_feature;

  A_local[main_index]=A[main_index];
  ...
  float threshold = A_local[main_index]
  float left_total=0.;
  float right_total=0.;
  for(int i=0; i<%(num_samples)d; i++){
    ...
    int class=A_local[index]>threshold;
